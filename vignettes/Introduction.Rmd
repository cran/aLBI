...
- **FreqTM**: Creates frequency table for fish length of multiple months using either a custom bin width or Wang's formula as described in `FrequencyTable` function.
...
```{r data_preparation for FrequencyTable}
library(readxl)
...
- **Pmega**: A numeric value representing the percentage of megaspawner.
...
### Output
The function returns a named vector with TSB40 and LSB25 values indicating stock status, and selectivity of the fishing practice.
...
## To read more detail 
Readers are encouraged to read the article published in *Fisheries Research* of this `aLBI` R Package to understand the functionality and methodolgy.
